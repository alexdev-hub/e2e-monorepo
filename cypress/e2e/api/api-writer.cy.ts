...
describe('Writer API', () => {
  it('assert schema for the first app from the list', () => {
    cy.getApplications().then((applications) => {
      const firstAppId: string = applications[0].id

      cy.getApplicationDetails(firstAppId).then((app) => {
        applicationSchema.parse(app)
        const { id, name, status } = app
  ...
  it('should generate text from prompt', () => {
    ...
    cy.generateText(requestBody).then((body) => {
      ...
      expect(choices[0].text.toLowerCase()).to.include('writer.com')
